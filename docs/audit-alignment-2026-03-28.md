...
| A11 | `local-only` label meaning | In branch-stacks.md: means "not yet submitted as PR to upstream ATAS", not "absent from origin". All active branches should exist on origin. | Added legend table to branch-stacks.md with all status labels defined | closed |
| D6 | `patch-registry.md` build stacks section | Exhaustive per-indicator key lists duplicated git log, went stale quickly | Replaced with compact table (purpose + key count per indicator); removed cherry-pick command sets | closed |
| E11 | ATAS X — `Editors/` WPF incompatibility | `LevelSettingsEditor` (used by **OHLCPlus**), `RangeEditor` (OIAnalyzer, SampleProperties), `CustomView` (sample) all use WPF UserControl/IValueConverter — incompatible with ATAS X. `SampleEditor` also affected. | OHLCPlus is the only production indicator affected. Audit + migration to native ATAS UI required before ATAS X publication. | open — deferred to OHLCPlus Phase 5/6 |
| G1 | Branch history audit + key integration workflow | Need to audit every active branch one by one: review commit history, verify Phase correctness, and define how new Resource keys are added going forward (automated by Claude or via explicit user command). | New Section G — next session | partial |
| G2 | `mn_trades-on-chart` origin cleanup | PLAT-3736 (3 commits Feb 17–26 2026) fully merged into Develop. Our `feat/tradesonchart` branches from current Develop tip → PLAT-3736 already included. Branch was an older intermediate state. | Deleted from origin 2026-03-29 | closed |
| G3 | `feat/SpeedOfTapeV2` classification | Original indicator (not an upstream port), not yet reviewed or ported to current .NET8 project. | Keep in origin; add to pending-work list before next publication cycle | open |
| G4 | `local/build/01-04` internal audit | Audited all 4 build branches: commit history, correctness, compat shims, resource sync tooling. See `docs/build-audit.md`. | Created build-audit.md (2026-03-30). Removed stale `docs/branch-stacks.md` from 04. Known deferred issues: Alpha/Cross in .sln, net10 runtime assumption. | partial |
| G5 | `docs/branch-stacks.md` contaminating 04 | Stale copy of build stack docs was committed to `local/build/03-version-shims` (commit `df02d00e`). Authoritative version is on meta/docs. | Removed via commit on `local/build/04-localization` (2026-03-30) | closed |
