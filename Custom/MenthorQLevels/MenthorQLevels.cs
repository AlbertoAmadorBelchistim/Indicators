...
namespace ATAS.Indicators.Technical
{
	...
	string sourceIdBase)
        ...
        // override first, then falling back to the chart instrument. Both
        // branches go through NormaliseTicker so a continuous chart ("ESM6"),
        // a specific contract ("ESM2026"), a micro ("MES") and a MenthorQ
        // paste header ("ES1!") all collapse to the same canonical root
        // ("ES"). Used for the API URL and as the chart-side input to
        // BuildTickerSuffix.
        private string ResolveTicker()
        {
            if (!string.IsNullOrWhiteSpace(_tickerOverride))
                return NormaliseTicker(_tickerOverride);

            var instrument = InstrumentInfo?.Instrument;
            return NormaliseTicker(instrument);
        }

        // Single source of truth for ticker normalisation. Applied to both
        // sides of the source-vs-chart equality check so a chart on ESM6 /
        // ESM2026 / MES matches a paste from "$ES:" or "$ES1!:" without the
        // renderer treating them as different instruments.
        //
        // Steps, applied in order:
        //   1. Trim + uppercase + null guard.
        //   2. Strip MenthorQ continuous suffix "1!" (ES1! → ES, NQ1! → NQ).
        //   3. Strip ATAS contract-month suffix: a single month-code letter
        //      from {F,G,H,J,K,M,N,Q,U,V,X,Z} immediately followed by 1..4
        //      digits at the end of the ticker (ESM6, ESM2026, NQH24 → ES,
        //      ES, NQ). Stops at one suffix; we never strip nested codes.
        //   4. Strip ATAS micro prefix: leading "M" on 3-character roots
        //      (MES → ES, MNQ → NQ, MYM → YM, M2K → 2K). Same heuristic
        //      ResolveTicker used previously, applied last so it runs on
        //      the post-suffix root (e.g. MESM6 → MES → ES).
        private static string NormaliseTicker(string raw)
        {
            if (string.IsNullOrWhiteSpace(raw))
                return string.Empty;
            ...
            // (2) MenthorQ continuous suffix
            if (s.Length > 2 && s.EndsWith("1!", StringComparison.Ordinal))
                s = s.Substring(0, s.Length - 2);

            // (3) ATAS contract-month suffix
            int n = s.Length;
            int digitEnd = n;
            while (digitEnd > 0 && s[digitEnd - 1] >= '0' && s[digitEnd - 1] <= '9')
                digitEnd--;
            int digitCount = n - digitEnd;
            if (digitCount >= 1 && digitCount <= 4 && digitEnd >= 2)
            {
                char monthChar = s[digitEnd - 1];
                if ("FGHJKMNQUVXZ".IndexOf(monthChar) >= 0)
                    s = s.Substring(0, digitEnd - 1);
            }
        ...
        private static Level[] BuildLevels(ParsedEntry[] entries, string chartTicker)
        {
            ...
            // Source info indexed by (price, category, rank, is0Dte) — the
            // same tuple DedupLabels uses as a label's identity. After
            // PickWinner returns a label, we look up its origin with the
            // matching tuple, so the ticker shown in the suffix is the
            // ticker of the visually-dominant label, not of whichever label
            // happened to be parsed first at that price. The first-write-wins
            // guard handles the rare case of two entries with identical
            // (Category, Rank, Is0Dte) at the same price (different feeds
            // converging) — we keep the earliest, mirroring DedupLabels.
            var sourceByLabelKey = new Dictionary<(decimal price, LevelCategory cat, int rank, bool is0Dte),
                                                  (string ticker, decimal? orig)>(entries.Length);
            ...
            for (int i = 0; i < entries.Length; i++)
            {
                ...
                for (int k = 0; k < e.Labels.Length; k++)
                {
                    ...
                    var key = (e.Price, l.Category, l.Rank, l.Is0Dte);
                    if (!sourceByLabelKey.ContainsKey(key))
                        sourceByLabelKey[key] = (e.SourceTicker, e.OriginalPrice);
                }
            }
            ...
            foreach (var kv in byPrice)
            {
                ...
                // Look up the source info that travelled with the winner.
                // Falls through to (empty, null) if the lookup misses — which
                // shouldn't happen given how we populated the map, but the
                // suffix builder treats an empty ticker as "no annotation"
                // and yields an empty string, so a miss degrades gracefully.
                var winnerKey = (price, winner.Category, winner.Rank, winner.Is0Dte);
                sourceByLabelKey.TryGetValue(winnerKey, out var winnerSource);

                var tickerSuffix = BuildTickerSuffix(winnerSource.ticker, winnerSource.orig, chartTicker);
        ...
        private static string BuildTickerSuffix(string sourceTicker, decimal? originalPrice, string chartTicker)
		{
            ...
            // Compare normalised forms so "$ES1!" / "$ES:" pastes both match
            // an ATAS chart on "ESM6" / "ESM2026" / "MES". The displayed
            // suffix keeps the original sourceTicker text — if the trader
            // pasted "$ES1!:", they see "(ES1! 5870.50)", not "(ES ...)".
            if (!string.IsNullOrWhiteSpace(chartTicker)
                && string.Equals(NormaliseTicker(sourceTicker), chartTicker, StringComparison.Ordinal))
                return string.Empty;
