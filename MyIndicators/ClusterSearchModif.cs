...
[DisplayName("Cluster Search Modif")]
...
public partial class ClusterSearch : Indicator
{
	...
	private bool CheckCluster(int bar, decimal price)
	{
		...
		// =======================================================================
		// Diagonal Imbalances Filters (exclusive pipeline)
		// =======================================================================
		if (CalcType == CalcMode.DiagonalImbalance)
		{
            var priceAbove = price;
            var priceBelow = price - InstrumentInfo.TickSize;

            if (_mergedLevels.TryGetValue(priceAbove, out var above) &&
                _mergedLevels.TryGetValue(priceBelow, out var below))
            {
                // BUY: Ask(P) vs Bid(P-1)
                bool buy =
                    below.Bid > 0 &&
                    above.Ask / below.Bid >= ImbalanceRatio &&
                    above.Ask - below.Bid >= MinVolumeDifference &&
                    above.Ask >= MinDominantVolume;

                // SELL: Bid(P-1) vs Ask(P)
                bool sell =
                    above.Ask > 0 &&
                    below.Bid / above.Ask >= ImbalanceRatio &&
                    below.Bid - above.Ask >= MinVolumeDifference &&
                    below.Bid >= MinDominantVolume;

                if (buy || sell)
                {
                    int stacked = 1;

                    for (int off = 1; off < ImbalanceStackedRange; off++)
                    {
                        var nextAbovePrice = priceAbove + off * InstrumentInfo.TickSize;   // P+off
                        var nextBelowPrice = priceBelow + off * InstrumentInfo.TickSize;   // P-1+off

                        if (!_mergedLevels.TryGetValue(nextAbovePrice, out var nextAbove) ||
                            !_mergedLevels.TryGetValue(nextBelowPrice, out var nextBelow))
                            break;

                        bool nextBuy =
                            nextBelow.Bid > 0 &&
                            nextAbove.Ask / nextBelow.Bid >= ImbalanceRatio &&
                            nextAbove.Ask - nextBelow.Bid >= MinVolumeDifference &&
                            nextAbove.Ask >= MinDominantVolume;

                        bool nextSell =
                            nextAbove.Ask > 0 &&
                            nextBelow.Bid / nextAbove.Ask >= ImbalanceRatio &&
                            nextBelow.Bid - nextAbove.Ask >= MinVolumeDifference &&
                            nextBelow.Bid >= MinDominantVolume;
                    ...
                    {
                        // Dibuja en el precio de la parte dominante:
                        var selectedInfo = buy ? above : below;
                        var selectedPrice = buy ? priceAbove : priceBelow;

                        selectedInfo.PriceSelectionColor = UseSeparateColors
                            ? (buy ? BuyImbalanceColor : SellImbalanceColor)
                            : _clusterPriceColor;

                        _validVolumeLevels[selectedPrice] = selectedInfo; // íojo! clave = precio dominante
                        return true;
                    }
                }
            }

            // No diagonal imbalance found for this price
            return false;
		}
    ...
    // Trigger recalc/redraw when a diagonal-imbalance setting changes.
    private void OnDiagonalImbalancePropertyChanged()
    {
        RecalculateValues();
        RedrawChart();
    }
