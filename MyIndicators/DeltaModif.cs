    ...
    public class DeltaModif : Indicator
    {
        ...
        public bool MinimizedMode
        {
            ...
            set
            {
                ...
                RaisePropertyChanged(nameof(MinimizedMode));
                RecalculateValues();
                UpdateDivergenceCandlesVisibility();
            }
        }
        ...
        public Filter UpAlert { get; set; } = new() { Enabled = false, Value = 0 };
        ...
        [DisplayFormat(DataFormatString = "F0")]
        public Filter DownAlert { get; set; } = new() { Enabled = false, Value = 0 };
        ...
        protected override void OnRender(RenderContext context, DrawingLayouts layout)
        {
            ...
            for (var i = FirstVisibleBarNumber; i <= LastVisibleBarNumber; i++)
            {
                ...
                if (MinimizedMode)
                {
                    value = _candles[i].Close > 0 ? _candles[i].Close : -_downCandles[i].Close;
                }
                else
                ...
                var strRect = new Rectangle(ChartInfo.GetXByBar(i), y, barWidth, strHeight);
