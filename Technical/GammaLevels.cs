...
namespace ATAS.Indicators.Technical
{
	...
	public sealed class GammaLevels : Indicator
	{
		...
		public GammaLevels()
		{
            ...
            AttachPen(_penPutWall);
            AttachPen(_penCallWall);
            AttachPen(_penVolTrigger);
            AttachPen(_penLargeGamma);
            AttachPen(_penCombo);
            AttachPen(_penZeroGamma);
            AttachPen(_penBlindLevel);
            AttachPen(_penDayMin);
            AttachPen(_penDayMax);
            AttachPen(_penRiskTrigger);
            AttachPen(_penLowerBand);
            AttachPen(_penUpperBand);
            AttachPen(_penOther);
            AttachPen(_pen0DteAccent);
        }
		#endregion
		...
		void SetPen(ref PenSettings field, PenSettings value)
		{
			...
			// Detach from previous instance changes
			DetachPen(field as INotifyPropertyChanged);

			field = value;

			// Attach to new instance changes
			AttachPen(field as INotifyPropertyChanged);

			_visualDirty = true;
			RedrawChart();
		}

		private void AttachPen(INotifyPropertyChanged? pen)
		{
			if (pen is null)
				return;

			pen.PropertyChanged -= OnPenSettingsChanged;
			pen.PropertyChanged += OnPenSettingsChanged;
		}

		private void DetachPen(INotifyPropertyChanged? pen)
		{
			if (pen is null)
				return;
		...
		private void OnPenSettingsChanged(object? sender, PropertyChangedEventArgs e)
		{
			_visualDirty = true;
			RedrawChart(); // en tu archivo esto llama a RecalculateValues()
		}
