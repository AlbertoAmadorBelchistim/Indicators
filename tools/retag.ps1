...
$OldRef   = "backup/prready-main-pre-rebase"
$NewRef   = "prready/main"
...
$baseNew = git merge-base $NewRef $Upstream
...
# Tags to move (only tags merged in old ref)
$tags = git tag --merged $OldRef

# Ensure temp dir exists for annotated tag messages
$tagDir = Join-Path $env:TEMP ("tagmsg_clusterstatistic_" + [Guid]::NewGuid().ToString("N"))
...
foreach ($t in $tags) {
    ...
    if ($tagType -eq "tag") {
        ...
        $tmp = Join-Path $tagDir ("tagmsg_" + $t + ".txt")
